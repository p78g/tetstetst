...
try {
    ...
    async function waitForGameStart(bots, mode) {
        ...
        runCheatMenu(bots, mode);
    }

    async function runCheatMenu(bots, mode) {
        if (bots.length === 0) {
        ...
        // Loop for multiple cheat executions
        ...
        while (continueRunning) {
            ...
            // Ask if user wants to run another cheat
            const runAnother = (await question('\nDo you want to run another cheat? (Y/N): ')).toLowerCase();
            if (runAnother !== 'y' && runAnother !== 'yes') {
                continueRunning = false;
            }
        }

        // Close all bot WebSockets and exit
        console.log(yellow('Closing connections and exiting...'));
